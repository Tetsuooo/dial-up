...
class MistPainter {
  constructor(sprite) {
    ...
    // Set initial alpha to ensure visibility
    this.sprite.alpha = 0.6;

    // Diagnostic logging
    console.log(`MistPainter initialized: Sprite dimensions ${this.sprite.width}x${this.sprite.height}, Position (${this.sprite.x}, ${this.sprite.y}), Alpha: ${this.sprite.alpha}`);

    // Create random movement vector
    const angle = Math.random() * Math.PI * 2;
    
    // Use exact speed specified (0.025) and apply in random direction
    const speed = 0.025;
    ...
    this.sprite.zIndex = 30; // Match the zIndex from mix.js
  ...
  updateSprite(delta) {
    ...
    if (!this.sprite || !this.sprite.texture) {
      console.warn("Mist sprite or texture is null");
      return;
    }
    ...
    if ((this.sprite.x + this.spriteData.xChange * delta > this.canvasWidth - margin) || 
      ...
      console.log("Mist changed horizontal direction");
    }
    ...
    if ((this.sprite.y + this.spriteData.yChange * delta > this.canvasHeight - margin) || 
      ...
      console.log("Mist changed vertical direction");
    }
    ...
    // Apply triangle wave with range controlled by * 0.1
    this.sprite.alpha = 0.4 + (triangleWave * 0.1);
    
    // Log position occasionally to debug movement
    if (Math.random() < 0.01) {
      console.log(`Mist position: (${this.sprite.x.toFixed(1)}, ${this.sprite.y.toFixed(1)}), velocity: (${this.spriteData.xChange.toFixed(3)}, ${this.spriteData.yChange.toFixed(3)})`);
    }
  }
}
